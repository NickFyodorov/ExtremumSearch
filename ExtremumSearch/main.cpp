...
int main() {
	...
	std::vector<Area*> Areas = { new NCube(Ranges), new NCube, new NCube };
	std::vector<Function*> Functions = { new TestFunc01(Areas[0]),new TestFunc02(Areas[0]), new TestFunc03(Areas[0]), new TestFunc04(Areas[0]), new TestFunc05(Areas[0]) };
	std::vector<OptimizationMethod*> OptMethods = { new NelderMead, new RandomSearch };
	std::vector<TerminalCondition*> Conditions = { new ConditionFDiff(eps), new ConditionXDiff(eps), new ConditionGrad(eps), new ConditionIter(count), new ConditionImprovement(count) };
	...
	while (next) {
		...
		int cond;
		if (buffer == "Iter") cond = 0;
		if (buffer == "Improvement") cond = 1;
		if (buffer == "fDiff") cond = 2;
		if (buffer == "xDiff") cond = 3;
		if (buffer == "Grad") cond = 4;
		...
		Result = OptMethods[method]->Optimize(Areas[area], Functions[func], Conditions[cond], FirstPoints[first_point]);
